...
Light2D::Light2D(const std::string& strName, const std::string& texName) : ARenderer(strName)
{
	this->pTexture = TextureManager::getInstance()->get(texName);
	this->mColor = { 255, 255, 255, 255 };
	this->size = Vector2D(64.0f, 64.0f);
	this->flipX = false;
	this->flipY = false;
}

void Light2D::perform()
{
